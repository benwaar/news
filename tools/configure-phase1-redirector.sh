...
EXECUTIONS_JSON=$(docker exec "$CONTAINER" /opt/keycloak/bin/kcadm.sh get "authentication/flows/$NEW_FLOW/executions" -r "$REALM")
...
if [[ -z "${EXEC_ID:-}" ]]; then
  ...
  EXECUTIONS_JSON=$(docker exec "$CONTAINER" /opt/keycloak/bin/kcadm.sh get "authentication/flows/$NEW_FLOW/executions" -r "$REALM")
  EXEC_ID=$(echo "$EXECUTIONS_JSON" | python3 -c 'import sys,json; j=json.load(sys.stdin); print(next((e.get("id","") for e in j if e.get("providerId")=="identity-provider-redirector"), ""))')
  [[ -z "$EXEC_ID" ]] && fail "Could not find execution id after creating identity-provider-redirector."
fi
...
docker exec "$CONTAINER" /opt/keycloak/bin/kcadm.sh update "authentication/executions/$EXEC_ID" -r "$REALM" -s requirement=ALTERNATIVE >/dev/null || true
